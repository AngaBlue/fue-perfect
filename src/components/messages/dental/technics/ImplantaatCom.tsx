...
export default function ImplantaatCom({
  state,
  setState,
}: {
  state: DentalState
  ...
  const changeImplantaatOptions: ChangeEventHandler<HTMLSelectElement> = (e) =>
    setState({ ...state, implantOptions: e.target.value })
  const changeImplantaatMaterials: ChangeEventHandler<HTMLSelectElement> = (
    e,
  ) => setState({ ...state, implantMaterials: e.target.value })
