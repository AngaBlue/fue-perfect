...
export default function Content(state: HairState) {
  ...
  useEffect(() => {
    import('!url-loader!./assets/logo.png').then(({ default: img }) =>
      setLogo(img),
    )
  }, [])
  ...
  return (
    <div className={styles.message} style={{ fontFamily: 'Sans-Serif' }}>
      <p>
        Geachte {state.gender === Gender.MALE ? 'heer' : 'mevrouw'}{' '}
      ...
      {state.inspection ? (
        <>
          <p>
            ...
            wij hebben uw aanvraag + foto&apos;s in goede orde ontvangen.
          </p>
          <p>
            Uw foto&apos;s / toelichting zijn beoordeeld en hieronder vindt u de
            analyse terug m.b.t uw Haartransplantatie behandeling, u bent altijd
            van harte welkom voor een vrijblijvend consult naar onze kliniek te
            Hoofddorp!
          </p>
          <p>
            * Aangezien de beoordeling d.m.v. foto&apos;s is vastgesteld,
            bestaat een kleine kans van afwijking na de definitieve onderzoek in
            onze kliniek.
          </p>
        </>
      ) : (
        <>
          ...
          <p>
            ...
            heeft op{' '}
            <strong>{format(state.date, 'PPP', { locale: nl })}</strong> een
            vooronderzoek ondergaan omtrent uw FUE Haartransplantatie
            behandeling.
          </p>
          <p>
      ...
      <p>
        ...
        {state.sessions === 2 && (
          <>
            <strong>Aantal grafts tweede sessie</strong>: {state.grafts[1]}{' '}
            grafts (niet verplicht)
            <br />
          </>
        )}
        ...
        <>
          <p>
            <strong style={{ color: '#c82613' }}>
              ...
              {state.sessions === 1 ? '' : 's'} in Nederland All-in €
            ...
            {state.sessions === 2 && (
              <>
                ...
                <strong style={{ textDecoration: 'underline' }}>
                  Eerste sessie: €{state.price[0][0]}{' '}
                  {!!state.discount && `(€${Math.abs(state.discount)} korting)`}
                </strong>
                <br />
                <span style={{ textDecoration: 'underline' }}>
                  <strong>Tweede sessie: €{state.price[0][1]} </strong>(na min
                  12 maanden genezingstijd, niet verplicht)
                </span>
              </>
            )}
          </p>
      ...
      {state.country !== Countries.NETHERLANDS && (
        <>
          <p>
            <strong style={{ color: '#c82613' }}>
              Kosten behandeling {state.sessions} sessie
              {state.sessions === 1 ? '' : 's'} in Turkije All-in €
            ...
            {state.sessions === 2 && (
              <>
                <br />
                <strong style={{ textDecoration: 'underline' }}>
                  Eerste sessie: €{state.price[1][0]}{' '}
                  {!!state.discount && `(€${Math.abs(state.discount)} korting)`}
                </strong>
                <br />
                <span style={{ textDecoration: 'underline' }}>
                  <strong>Tweede sessie: €{state.price[1][1]} </strong>(na min
                  12 maanden genezingstijd, niet verplicht)
                </span>
              </>
            )}
