...
import { Country, Discounts, Gender, Grafts, HairQuality, type HairState, PRPPrices, Prices, Technique } from './data';
...
export default function Form({ state, setState }: { state: HairState; setState: Dispatch<SetStateAction<HairState>> }) {
	...
	useEffect(() => {
		...
		if (state.sessions === 2) {
			price[Country.NETHERLANDS][1] =
				state.priceOverride[Country.NETHERLANDS][1] || Prices[Country.NETHERLANDS][Grafts.first.findIndex(g => g === state.grafts[1])];
		}
	...
	}, [state.priceOverride, state.sessions, state.grafts, state.country, state.discount, state, setState]);
	...
	return (
		<SimpleGrid columns={[1, null, 2, 3, 4]} spacing={4}>
			...
			<Box>
				...
				<RadioGroup value={state.sessions.toString()} onChange={s => setState({ ...state, sessions: Number(s) as typeof state.sessions })}>
					<VStack align='left'>
			...
			<Box>
				...
				<RadioGroup value={state.discount.toString()} onChange={d => setState({ ...state, discount: Number(d) as typeof state.discount })}>
					<VStack align='left'>
						{Discounts.map(v => (
			...
			<Box>
				...
				<InputGroup>
					<Textarea placeholder='-' value={state.opmerkingNotes} onChange={e => setState({ ...state, opmerkingNotes: e.target.value })} />
				</InputGroup>
			</Box>
			<Box>
