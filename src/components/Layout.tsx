...
import { type Dispatch, type FunctionComponent, type ReactNode, type SetStateAction, useEffect, useState } from 'react';
...
export default function Layout<State>({ credentials, content, form: Form, subject, name, state, setState }: LayoutProps<State>) {
	...
	useEffect(() => setHtml(ReactDOMServer.renderToStaticMarkup(<TypesafeI18n locale={locale}>{content}</TypesafeI18n>)), [content, locale]);
